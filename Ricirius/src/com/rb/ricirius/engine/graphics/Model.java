...
import com.rb.ricirius.engine.util.BufferUtil;
import com.rb.ricirius.engine.util.LangUtil;
...
public class Model {
	...
	public Model(Vertex[] vertices, int[] indices, boolean calcNormals) {
		initModelData();
	...
	private Model loadModel(String fileName) {
		...
		if (!ext.equals("obj")) {
			System.err.println("Error: Invalid file format " + ext + ". File format should be obj");
		...
		try {
			...
			while ((line = modelReader.readLine()) != null) {
				...
				tokens = LangUtil.removeEmptyStrings(tokens);
			...
			addVertices(vertexData, LangUtil.toIntArray(indexData), true);
